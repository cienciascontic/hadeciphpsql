...
// Armar array de escuelas
$escuelas = [
  [  "nombre" => "(clic para seleccionar su escuela de la lista)",
    "direccion" => ""
  ],

[  "nombre" => "San Juan",
  "direccion" => "Cucha-cucha 123"
],

[  "nombre" => "Nacional",
  "direccion" => "Piedras 555"
],

[  "nombre" => "Etcheverry Boneo",
  "direccion" => "Juncal 2068"
],
];

// fin array de escuelas
if ($_POST != [])
{
  $usuarioNuevo[] = armarUsuario();
  $jsondeusuarios = json_encode($usuarioNuevo);
  //var_dump(file_get_contents("usuarios.json") != "");
  if (file_get_contents("usuarios.json") == "")
  {
    file_put_contents("usuarios.json",$jsondeusuarios);
  }
  else
  {
    $usuariosExistentes = json_decode(file_get_contents("usuarios.json"), true);
    $usuariosExistentes [] = $usuarioNuevo[0];
    file_put_contents("usuarios.json",json_encode($usuariosExistentes));
  }
}
 ?>
...
<div class="row">
  <div class="col">
    <input type="email" name="correo" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Ingrese su dirección de email">
  </div>
  <div class="col">
      <select type="text" name="escuela" class="form-control" placeholder="Seleccione su escuela">
        <!-- php para elegir la escuela de un combo box -->
        <?php
        for ($i=0; $i < count($escuelas); $i++) { ?>
          <option value=""><?php echo $escuelas[$i]["nombre"];?></option>
        <?php } ?>
<!-- fin de php para elegir la escuela de un combo box -->
      </select>
  </div>
</div>
